...
class A3CAgent(base_agent.BaseAgent):
  ...
  def setup(self, obs_spec, actions_spec, sess, summary_writer):
    super(A3CAgent,self).setup(obs_spec,actions_spec)
    self.sess = sess
  ...
  def step(self, obs):
    ...
    info[0, obs.observation['available_actions']] = 1 #mask for available actions
    ...
    for arg in actions.FUNCTIONS[act_id].args:
      if arg.name in ('screen', 'minimap', 'screen2'):
        act_args.append([target[1], target[0]]) #use (y,x)->(height,width)
      else:
  ...
  def update(self, rbs, disc, lr, cter):
    ...
    params:
      rbs: replay buffer
      disc: discount factor(lambda)
      lr: learning rate
      cter: counter/episodes
    ------
    rtype:
      None
    '''
    ...
    rbs.reverse()
    for i, [obs, action, _next_obs] in enumerate(rbs):
      minimap = np.array(obs.observation['feature_minimap'], dtype=np.float32)
      minimap = np.expand_dims(U.preprocess_minimap(minimap), axis=0)
      screen = np.array(obs.observation['feature_screen'], dtype=np.float32)
      screen = np.expand_dims(U.preprocess_screen(screen), axis=0)
  ...
  def load_model(self, path):
    ...
    self.saver.restore(self.sess, ckpt.model_checkpoint_path)  #pylint: disable=E1101
    self.episodes = int(ckpt.model_checkpoint_path.split('-')[-1])#pylint: disable=E1101
